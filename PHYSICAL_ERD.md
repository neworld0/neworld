...
기준: 현재 모델과 마이그레이션(`common` 0013, `neworld` 0025), 그리고 운영 설정 [`config/settings/prod.py`](config/settings/prod.py).  
...
erDiagram
    ...
    auth_user o|--o{ neworld_customer : "author_id"
    auth_group o|--o{ neworld_customer : "group_id"
    neworld_customer ||--o{ neworld_activity : "customer_id"
    auth_user o|--o{ neworld_activity : "author_id"
    auth_group o|--o{ neworld_activity : "group_id"
    ...
    neworld_customer o|--o{ neworld_comment : "customer_id"
    neworld_activity o|--o{ neworld_comment : "activity_id"

    auth_user o|--o{ neworld_gpt : "author_id"
    auth_group o|--o{ neworld_gpt : "group_id"
    neworld_gpt ||--o{ neworld_gptanswer : "gpt_id"
```
...
| `neworld_customer` | `id integer`, `author_id integer NULL`, `group_id integer NULL`, `area text NULL`, `name text`, `keyman text`, `position text`, `grade varchar(2)`, `tel text NULL`, `address text NULL`, `email varchar(254) NULL`, `remark text NULL`, `create_date timestamptz`, `modify_date timestamptz NULL` | PK `id`; FK `author_id → auth_user`, `group_id → auth_group` |
| `neworld_activity` | `id integer`, `customer_id integer`, `author_id integer NULL`, `group_id integer NULL`, `content text`, `create_date timestamptz`, `modify_date timestamptz NULL` | PK `id`; FK `customer_id → neworld_customer`, `author_id → auth_user`, `group_id → auth_group` |
| `neworld_comment` | `id integer`, `author_id integer NULL`, `group_id integer NULL`, `question_id integer NULL`, `answer_id integer NULL`, `meditation_id integer NULL`, `research_id integer NULL`, `customer_id integer NULL`, `activity_id integer NULL`, `content text`, `create_date timestamptz`, `modify_date timestamptz NULL` | PK `id`; 각 `*_id`는 해당 테이블 FK; CHECK `comment_exactly_one_target` |
| `neworld_gpt` | `id integer`, `author_id integer NULL`, `group_id integer NULL`, `content text`, `create_date timestamptz`, `modify_date timestamptz NULL` | PK `id`; FK `author_id → auth_user`, `group_id → auth_group` |
| `neworld_gptanswer` | `id integer`, `gpt_id integer`, `content text`, `create_date timestamptz` | PK `id`; FK `gpt_id → neworld_gpt` |
...
| `neworld_customer_voter` | `id integer`, `customer_id integer`, `user_id integer` | PK `id`; FK 2개; UQ (`customer_id`, `user_id`) |
| `neworld_activity_voter` | `id integer`, `activity_id integer`, `user_id integer` | PK `id`; FK 2개; UQ (`activity_id`, `user_id`) |
| `neworld_gpt_voter` | `id integer`, `gpt_id integer`, `user_id integer` | PK `id`; FK 2개; UQ (`gpt_id`, `user_id`) |
| `neworld_gptanswer_voter` | `id integer`, `gptanswer_id integer`, `user_id integer` | PK `id`; FK 2개; UQ (`gptanswer_id`, `user_id`) |
...
- `neworld_comment`는 `comment_exactly_one_target` CHECK 제약으로 댓글 대상 FK 6개 중 정확히 하나만 채워지도록 강제한다.
